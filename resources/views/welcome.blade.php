  ...
  <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-indigo-600 via-purple-600 to-fuchsia-600 p-5 sm:p-6 shadow-xl">
    <div class="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
      ...
      <div class="flex flex-wrap gap-2">
        ...
        {{-- ⬇️ NUEVOS: exportar usuarios --}}
        <a href="{{ route('users.export', ['format' => 'json']) }}"
           class="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-emerald-500/95 text-white hover:bg-emerald-600 shadow-sm">
          <i class="fa-solid fa-download"></i> Usuarios JSON
        </a>
        <a href="{{ route('users.export', ['format' => 'xml']) }}"
           class="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-white/10 text-white hover:bg-white/20 ring-1 ring-white/30">
          <i class="fa-solid fa-file-code"></i> Usuarios XML
        </a>
      </div>
    </div>
...
@section('content')
  ...
  <div class="grid grid-cols-1 xl:grid-cols-3 gap-4 mt-5">
    ...
    <div class="rounded-2xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 shadow-md">
      ...
      {{-- Acción: exportar usuarios (también aquí) --}}
      <div class="mt-4 flex flex-wrap gap-2">
        {{-- Ver en navegador (abre nueva pestaña) --}}
        <a href="{{ route('users.export', ['format' => 'json', 'preview' => 1]) }}" target="_blank" rel="noopener"
        class="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-white/10 text-white hover:bg-white/20 ring-1 ring-white/30">
        <i class="fa-solid fa-eye"></i> Ver JSON
        </a>
        <a href="{{ route('users.export', ['format' => 'xml', 'preview' => 1]) }}" target="_blank" rel="noopener"
        class="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-white/10 text-white hover:bg-white/20 ring-1 ring-white/30">
        <i class="fa-solid fa-eye"></i> Ver XML
        </a>
      </div>
    </div>
...
@section('scripts')
  <script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // Datos demo para sparklines (compactos)
