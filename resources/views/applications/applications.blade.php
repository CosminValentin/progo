...
@section('content')
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    <div class="rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 shadow">
      ...
      <div class="mt-1 flex items-baseline gap-2">
        <div class="text-2xl font-semibold text-gray-800 dark:text-slate-100">{{ number_format($applications->total()) }}</div>
    ...
    <div class="rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 shadow">
      ...
      <div class="mt-1 text-2xl font-semibold text-gray-800 dark:text-slate-100">{{ $applications->currentPage() }}/{{ $applications->lastPage() }}</div>
    </div>
    <div class="rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 shadow">
      <div class="text-sm text-gray-500 dark:text-slate-400">Mostrando</div>
      <div class="mt-1 text-2xl font-semibold text-gray-800 dark:text-slate-100">{{ $applications->count() }}</div>
    </div>
    <div class="rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 shadow">
  ...
  <!-- Buscador -->
  <form method="GET" action="{{ route('applications') }}" class="mb-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
    <div class="col-span-2">
      <div class="relative">
        <input type="text" name="q" value="{{ $q ?? '' }}" placeholder="Buscar por participante, oferta o estado…"
        ...
        @forelse($applications as $a)
          <tr class="hover:bg-indigo-50 dark:hover:bg-slate-700/30 transition">
            ...
            <td class="px-5 py-4 text-gray-600 dark:text-slate-400 hidden md:table-cell">
              {{ $a->offer->titulo ?? $a->offer->nombre ?? ('Oferta #'.$a->offer_id) }}
            </td>
            <td class="px-5 py-4">
              <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs {{ $a->estado_badge_classes }}">
                  ...
                  <div x-transition class="relative w-full max-w-md rounded-xl bg-white dark:bg-slate-800 shadow-2xl ring-1 ring-black/10 dark:ring-white/10">
                    <div class="px-6 py-5 border-b border-gray-200 dark:border-slate-700">
                      ...
                      <p class="mt-2 text-sm text-center text-gray-600 dark:text-slate-300">
                        ¿Eliminar candidatura de <strong>{{ $a->participant->nombre ?? 'participante' }}</strong>?
                      </p>
                    </div>
