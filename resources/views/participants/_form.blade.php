  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
    <div>
      ...
      <input id="fecha_alta_prog" name="fecha_alta_prog" type="date" required
             ...
             class="w-full rounded-lg border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2.5 shadow-inner focus:ring-2 focus:ring-emerald-500 focus:outline-none transition">
...
<div class="mt-6 rounded-xl border border-fuchsia-100 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
  ...
  <div class="mb-5">
    <label for="notas" class="block text-xs font-medium text-gray-700 dark:text-slate-300 mb-1">
      Observaciones (generales)
    </label>
    <textarea id="notas" name="notas" rows="4"
              placeholder="Observaciones, detalles…"
              class="w-full rounded-lg border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2.5 shadow-inner focus:ring-2 focus:ring-fuchsia-500 focus:outline-none transition resize-y">{{ oldOrModel('notas', $participant) }}</textarea>
    @error('notas') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
  </div>

  <div>
    <label for="observaciones2" class="block text-xs font-medium text-gray-700 dark:text-slate-300 mb-1">
      Observaciones del trabajador (internas)
    </label>
    <textarea id="observaciones2" name="observaciones2" rows="4"
              placeholder="Información interna del trabajador, seguimiento, acuerdos, etc."
              class="w-full rounded-lg border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2.5 shadow-inner focus:ring-2 focus:ring-fuchsia-500 focus:outline-none transition resize-y">{{ oldOrModel('observaciones2', $participant) }}</textarea>
