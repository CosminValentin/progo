...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
  <div class="rounded-xl border border-gray-200 dark:border-slate-700 bg-gradient-to-r from-indigo-50 to-indigo-100 dark:from-slate-700/50 dark:to-slate-800 p-5 shadow-lg hover:shadow-2xl transition">
    ...
    <div class="mt-3 flex items-baseline gap-2">
      <div class="text-3xl font-semibold text-gray-800 dark:text-slate-100">{{ number_format($participants->total()) }}</div>
  ...
  <div class="rounded-xl border border-gray-200 dark:border-slate-700 bg-gradient-to-r from-blue-50 to-blue-100 dark:from-slate-700/50 dark:to-slate-800 p-5 shadow-lg hover:shadow-2xl transition">
    ...
    <div class="mt-3 text-3xl font-semibold text-gray-800 dark:text-slate-100">{{ $participants->currentPage() }}/{{ $participants->lastPage() }}</div>
  </div>
  ...
  <div class="rounded-xl border border-gray-200 dark:border-slate-700 bg-gradient-to-r from-green-50 to-green-100 dark:from-slate-700/50 dark:to-slate-800 p-5 shadow-lg hover:shadow-2xl transition">
    ...
    <div class="mt-3 text-3xl font-semibold text-gray-800 dark:text-slate-100">{{ $participants->count() }}</div>
  </div>
...
<!-- Search Form -->
<form method="GET" action="{{ route('participants') }}" class="mb-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
      ...
      <tbody class="divide-y divide-gray-100 dark:divide-slate-700 text-sm">
        @forelse($participants as $p)
          <tr class="hover:bg-indigo-50 dark:hover:bg-slate-700/30 transition">
            <td class="px-5 py-4 font-semibold text-indigo-700 dark:text-indigo-300">{{ $p->dni_nie }}</td>
            <td class="px-5 py-4 text-gray-700 dark:text-slate-300">{{ $p->nombre }}</td>
            <td class="px-5 py-4 text-gray-600 dark:text-slate-400 hidden md:table-cell">{{ $p->email }}</td>
            <td class="px-5 py-4 text-gray-600 dark:text-slate-400 hidden lg:table-cell">{{ $p->provincia }}</td>
            <td class="px-5 py-4">
        ...
        @empty
          <tr>
            <td colspan="5" class="px-5 py-4 text-center text-gray-600 dark:text-slate-400">No se encontraron participantes.</td>
          </tr>
        @endforelse
      </tbody>
    </table>
