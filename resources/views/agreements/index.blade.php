...
@section('header')
  <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
    <div>
      <h1 class="text-3xl font-bold text-indigo-700 dark:text-indigo-400">Convenios</h1>
...
@section('content')
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
    <div class="rounded-2xl p-6 text-white bg-gradient-to-br from-indigo-500 via-indigo-600 to-purple-600 shadow">
      <p class="text-sm opacity-80">Total convenios</p>
  ...
  <form id="agSearchForm" class="mb-6 grid grid-cols-1 sm:grid-cols-3 gap-4" onsubmit="return false;">
    <div class="col-span-2">
      <div class="relative">
        <input type="text" id="agSearchInput" placeholder="Buscar por empresa, fechas, firmado, documento…"
    ...
    <div class="flex gap-3">
      <button type="button" id="agSearchButton"
      ...
      <button type="button" id="agClearButton"
  ...
  <div class="rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-xl overflow-auto">
    <table id="agreementsTable" class="min-w-full table-auto border-collapse">
      <thead class="bg-gradient-to-r from-indigo-50 to-indigo-100 dark:from-slate-700/50 dark:to-slate-800 text-xs font-semibold uppercase text-indigo-700 dark:text-slate-200">
        <tr>
          <th class="px-4 py-3 text-left w-32">Fecha firma</th>
          <th class="px-4 py-3 text-left w-64">Empresa</th>
          <th class="px-4 py-3 text-left w-40">Validez</th>
          <th class="px-4 py-3 text-left w-28">Agencia</th>
          <th class="px-4 py-3 text-left w-28">Empresa</th>
          <th class="px-4 py-3 text-left w-40">Documento</th>
      ...
      <tbody class="divide-y divide-gray-100 dark:divide-slate-700 text-sm">
        @forelse($agreements as $a)
          <tr class="hover:bg-indigo-50 dark:hover:bg-slate-700/30 transition">
            <td class="px-4 py-3 whitespace-nowrap">{{ optional($a->fecha_firma)->format('d/m/Y') }}</td>
            <td class="px-4 py-3 truncate max-w-[280px]">
              {{ $a->company?->nombre ?? '—' }}
              @if($a->vigente)
                <span class="ml-2 inline-flex items-center px-2 py-0.5 text-[10px] rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-200">Vigente</span>
              @endif
            </td>
            <td class="px-4 py-3">
              {{ $a->validez_desde?->format('d/m/Y') ?? '—' }} — {{ $a->validez_hasta?->format('d/m/Y') ?? '—' }}
            </td>
            <td class="px-4 py-3">
              @if($a->firmado_agencia) <span class="text-emerald-600 font-bold">✔</span> @else <span class="text-rose-600 font-bold">✖</span> @endif
            </td>
            <td class="px-4 py-3">
              @if($a->firmado_empresa) <span class="text-emerald-600 font-bold">✔</span> @else <span class="text-rose-600 font-bold">✖</span> @endif
            </td>
            <td class="px-4 py-3 truncate max-w-[240px]">{{ $a->pdf?->nombre_archivo ?? '—' }}</td>
                  ...
                  <div x-transition class="relative w-full max-w-md rounded-xl bg-white dark:bg-slate-800 shadow-2xl ring-1 ring-black/10 dark:ring-white/10">
                    <div class="px-6 py-5 border-b border-gray-200 dark:border-slate-700">
                      ...
                      <p class="mt-2 text-sm text-center text-gray-600 dark:text-slate-300">
                        ¿Eliminar el convenio de <strong>{{ $a->company?->nombre ?? '—' }}</strong>?
                      </p>
                    </div>
                    <div class="flex items-center justify-center gap-4 p-6">
                      <form method="POST" action="{{ route('agreements.destroy', $a) }}">
                        @csrf @method('DELETE')
                      ...
                      <button type="button" @click="open=false" class="px-6 py-2 text-gray-700 dark:text-slate-200 bg-gray-100 hover:bg-gray-200 dark:bg-slate-600 dark:hover:bg-slate-700 rounded-lg">Cancelar</button>
                    </div>
                  </div>
                </div>
              </div>
        ...
        @empty
          <tr><td colspan="7" class="px-5 py-6 text-center text-gray-600 dark:text-slate-400">No hay convenios.</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
...
@section('scripts')
<script>
  // Buscador instantáneo estilo participantes
  document.addEventListener('DOMContentLoaded', () => {
    const input  = document.querySelector('#agSearchInput');
    const btn    = document.querySelector('#agSearchButton');
    const clear  = document.querySelector('#agClearButton');
