...
@section('header')
  <div class="flex items-end justify-between">
    <div>
      <h1 class="text-3xl font-bold text-indigo-700 dark:text-indigo-400">Editar contrato #{{ $contract->id }}</h1>
      <p class="text-sm text-gray-600 dark:text-slate-400">Actualiza y guarda.</p>
    </div>
...
@section('content')
  ...
  <form method="POST" action="{{ route('contracts.update', $contract) }}"
    ...
    @csrf @method('PUT')
    @include('contracts._form', ['contract'=>$contract, 'participants'=>$participants, 'companies'=>$companies, 'offers'=>$offers, 'documents'=>$documents])
